...
import { Plus, Search, Youtube, Linkedin, Twitter, FileText, Eye, ThumbsUp, MessageCircle, Share2 } from 'lucide-react';
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-2xl font-bold">Competitor Content Log</h1>
        <Button onClick={() => setShowNew(true)}><Plus className="mr-2 h-4 w-4" /> Log Content</Button>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Select value={compFilter} onValueChange={setCompFilter}>
          <SelectTrigger className="w-[160px]"><SelectValue placeholder="Competitor" /></SelectTrigger>
          <SelectContent>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <Card><CardContent className="p-8 text-center text-muted-foreground">No competitor content logged yet.</CardContent></Card>
        ) : filtered.map(c => {
            ...
            <Card key={c.id} className="cursor-pointer hover:bg-accent/30 transition-colors" onClick={() => setSelected(c)}>
              <CardContent className="p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{(c as any).competitors?.name} · {c.published_at ? new Date(c.published_at).toLocaleDateString() : 'No date'}</p>
                </div>
                ...
                {c.is_analyzed && <Badge variant="outline" className="text-xs text-status-published border-status-published/30">Analyzed</Badge>}
              </CardContent>
            </Card>
          );
        })}
          ...
          {selected && (
            <>
              ...
              <div className="space-y-3">
                ...
                {selected.analysis_notes && <div><p className="text-xs text-muted-foreground mb-1">Analysis</p><p className="text-sm">{selected.analysis_notes}</p></div>}
                <Button variant="secondary" className="w-full">Analyze (Coming Soon)</Button>
              </div>
            </>
          )}
        </DialogContent>
      ...
      <Dialog open={showNew} onOpenChange={setShowNew}>
        <DialogContent>
          <DialogHeader><DialogTitle>Log Competitor Content</DialogTitle></DialogHeader>
          <div className="space-y-3">
            ...
            <Textarea placeholder="Content / Transcript" value={form.content_body} onChange={(e) => setForm({ ...form, content_body: e.target.value })} />
            <Button onClick={createContent} disabled={!form.competitor_id || !form.title} className="w-full">Log Content</Button>
          </div>
        </DialogContent>
      </Dialog>
    </motion.div>
